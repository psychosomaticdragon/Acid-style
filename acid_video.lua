...
cmd:option('-start_number', 153, 'frame number to start at')
...
cmd:option('-content_layers', 'relu0,relu1,relu2,relu3,relu5,relu6,relu7,relu8,relu9', 'layers for content')
...
local function main(params)
  ...
  local content_image = image.load(string.format(params.content_image, params.start_number), 3)
  ...
  local num_calls = 0
  local frame = params.start_number
  ...
  local function feval(x)
    num_calls = num_calls + 1 
		...
		if should_wew then
			local frame_image = image.load(string.format(params.content_image, frame), 3)
			print('processing: ', string.format(params.content_image, frame))
			frame_image = image.scale(preprocess(frame_image):float(), params.image_size, 'bicubic')
			frame = frame + 1
			...
			warp_guide = build_warpmap(xnorm, rotation, false, true, params.warp_thresh, params.warp_breathe, params.magmap_ratio, params.rotational)
